""" 
Do FARM hospitalisations prediction using our implementation of the FARM model.
"""
...
for index in range(n_backtest_days):
    ...
    X_train = Xs[furthest_lookback_day:todays_date,:,target_feature] 
    X_train_diff = X_train[1:] - X_train[:-1]

    # Compute factor and idiosyncratic design matrices 
    evals, evecs = eigs(X_train_diff.T@X_train_diff, k = 1) 
    evals = evals.real 
    evecs = evecs.real
    ...
    lasso_reg_object = Lasso(alpha = 0.005,fit_intercept=False)
