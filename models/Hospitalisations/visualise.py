# Plot the predicted log admissions against the ground truth 

#!/usr/bin/env python3
# USAGE: ./visualise.py <DESIGN_MATRIX>.csv backtesting_config.yaml predictions-1.csv predictions-2.csv ... 
...
dates = pd.date_range(start='2021-07-21', end='2021-07-31', freq='D')

fig_gt_pred = go.Figure() 
names = ["Ground truth","NIRVAR" ,"FARM", "FNETS","GNAR"] 
colors = [
...
for i in range(len(predictions_list) + 1):
    fig_gt_pred.add_trace(go.Scatter(x=[s+1 for s in range(10)], y=gt_compare_list[i], mode='lines', name=names[i],line=dict(color=colors[i])))

fig_gt_pred.update_layout(
    xaxis_title='Day',
    yaxis_title='Ventilation admissions (log transformed)', 
    xaxis=dict(
