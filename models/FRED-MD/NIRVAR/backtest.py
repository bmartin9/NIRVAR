""" 
Predict the monthly Industrial Production using NIRVAR.
...
for index in range(n_backtest_days):
    ...
    subset_to_predict = np.array([5]) # only predict IP 
    ols_params = trainer.ols_parameters_subset(neighbours,subset_to_predict=subset_to_predict)
    labels_hat[index] = labels[target_feature]

    #predict next day returns 
    # todays_Xs = Xs[todays_date-1,:,:]
    todays_Xs = X_train[-1,:,:]
