# Predict Industrial Production using GNAR applied to FRED-MD. Use NIRVAR defined groups. 
...
adjacency_array <- aperm(
  array(adjacency_matrix, dim = c(N, n_backtest_days, N)),
  perm = c(2, 1, 3)
)
...
s <- matrix(NA, nrow = n_backtest_days, ncol = 1)
...
for (index in 1:n_backtest_days){
    ...
    sic_net <- graph_from_adjacency_matrix(adjacency_array[index,,], 'undirected')
    gnar_net <- igraphtoGNAR(sic_net)
    fit_gnar <- GNARfit(vts = X_train_scaled, 
                    net = gnar_net, 
                    alphaOrder = 1,
                    betaOrder = c(1),
                    globalalpha = FALSE
