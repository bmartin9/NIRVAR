""" 
Create NIRVAR adjacency matrices for each backtest day. Output is a csv file with NT rows and N columns.
"""

#!/usr/bin/env python
# USAGE: python create-adjacencies.py fred-balanced.csv backtesting_config.yaml
...
A_matrices = np.zeros((n_backtest_days,N,N)) #adjacency matrices
...
for index in range(n_backtest_days):
    ...
    # print(neighbours[0,:5,:15])
    A_matrices[index] = neighbours[0,:,:]
...
###### OUTPUT adjacencies TO FILE ###### 
A_matrices = np.reshape(A_matrices, (int(n_backtest_days*N),N))
A_hat_path = f"adjcacencies_NIRVAR_estimated-{PBS_ARRAY_INDEX}.csv"
np.savetxt(A_hat_path, A_matrices, delimiter=',', fmt='%d')
